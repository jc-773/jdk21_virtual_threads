...
public class AggregatorService {
    ...
    public ProductDTO getProductDto(int id) throws ExecutionException, InterruptedException {
        ...
        var product = CompletableFuture.supplyAsync(() -> Client.getProduct(id), executor).exceptionally(ex -> "Try a different product Id");
        //Another task gets it's own thread
        var rating = CompletableFuture.supplyAsync(() -> Client.getRating(id), executor).exceptionally(ex -> "Try a different rating Id");
